...
Hashmap *hashmapCreate(size_t initialCapacity,
    ...
    Hashmap *map = real_malloc(sizeof(Hashmap));
    if (map == NULL) {
        return NULL;
    }
    ...
    map->buckets = real_calloc(map->bucketCount, sizeof(Entry *));
    if (map->buckets == NULL) {
        real_free(map);
        return NULL;
    }
...
static void expandIfNecessary(Hashmap *map) {
    ...
    if (map->size > (map->bucketCount * 3 / 4)) {
        ...
        Entry **newBuckets = real_calloc(newBucketCount, sizeof(Entry *));
        if (newBuckets == NULL) {
            // Abort expansion.
            return;
        }
...
static Entry *createEntry(void *key, int hash, void *value) {
    Entry *entry = real_malloc(sizeof(Entry));
    if (entry == NULL) {
        return NULL;
    }
    entry->key = key;
...
static inline bool equalKeys(void *keyA, size_t hashA, void *keyB, size_t hashB,
